...
def login(
    ...
    user = next(
        (c for c in candidates if c.hashed_password and verify_password(password, c.hashed_password)),
        None,
    )
    if not user:
    ...
    _record_login_attempt(
        db, user_id=user.id, identifier=identifier, method=method, success=True,
